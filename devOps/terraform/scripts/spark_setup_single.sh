...
cassandra_JAR_PATH=${spark_lib}${cassandra_JAR}
sed -i '21i spark.hadoop.fs.s3a.impl org.apache.hadoop.fs.s3a.S3AFileSystem' ${SPARK_HOME}/conf/spark-defaults.conf
sed -i '22i spark.executor.extraClassPath '"${aws_java_sdk_jar}"':'"${hadoop_aws_jar}"':'"${cassandra_JAR_PATH}"'' ${SPARK_HOME}/conf/spark-defaults.conf
sed -i '23i spark.driver.extraClassPath '"${aws_java_sdk_jar}"':'"${hadoop_aws_jar}"':'"${cassandra_JAR_PATH}"'' ${SPARK_HOME}/conf/spark-defaults.conf
